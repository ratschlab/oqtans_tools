...
state_model(STATES.start).successors = [STATES.negative STATES.positive];
state_model(STATES.start).trans_scores = [0 0];
state_model(STATES.negative).successors = [STATES.negative STATES.positive STATES.stop];
state_model(STATES.negative).trans_scores = [1 1 0];
state_model(STATES.positive).successors = [STATES.positive STATES.negative STATES.stop];
...
%    = [[1:PAR.num_features]', STATES.start*ones(PAR.num_features,1)];
state_model(STATES.negative).feature_scores = [[1:PAR.num_features]', STATES.negative*ones(PAR.num_features,1)];
% scoring function sharing test
%    = [[1:PAR.num_features]', STATES.start*ones(PAR.num_features,1)];
state_model(STATES.positive).feature_scores = [[1:PAR.num_features]', STATES.positive*ones(PAR.num_features,1)];
...
state_model(STATES.negative).monot_scores = -ones(sum(state_model(STATES.negative).learn_scores),1);
state_model(STATES.positive).monot_scores = ones(sum(state_model(STATES.positive).learn_scores),1);
...
% coupling test
%    = [[1:PAR.num_features]', STATES.negative*ones(PAR.num_features,1)];
state_model(STATES.negative).score_coupling = zeros(sum(state_model(STATES.negative).learn_scores),2);
state_model(STATES.positive).score_coupling = zeros(sum(state_model(STATES.positive).learn_scores),2);
