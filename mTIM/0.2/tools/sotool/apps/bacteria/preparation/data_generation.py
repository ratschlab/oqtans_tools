...
class GFFEntry(object):
    ...
    def __init__(self, row):
        ...
        description_tokens = self.id.split(";")
        for token in description_tokens:
            key = token.split("=")[0]
            value = token.split("=")[1]
            self.description_map[key] = value
...
def parse_ptt_file(fn):
    ...
    for line_num, line in enumerate(file(fn)):
        ...
        assert not synoym_to_cog.has_key(syn)

        synoym_to_cog[syn] = cog
        
    return synoym_to_cog
...
def parse_gff_file(input_fn):
    ...
    ptt_fn = input_fn.replace(".gff", ".ptt")
...
if __name__ == '__main__':
    fn = "/fml/ag-raetsch/home/cwidmer/Documents/phd/projects/so-mtl/sotool/data/Acidobacterium_capsulatum_ATCC_51196_uid59127/NC_012483.ptt"
    print parse_ptt_file(fn)
