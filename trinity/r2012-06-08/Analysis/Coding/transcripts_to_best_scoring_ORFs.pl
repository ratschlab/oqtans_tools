...
use vars qw ($opt_t $opt_m $opt_G $opt_h $opt_v $opt_C $opt_T $opt_R $opt_P $opt_S $opt_B);


my $UTIL_DIR = "$FindBin::Bin/util";


&getopts ('t:m:G:hvCT:R:P:SB');
...
if ($opt_B) {
    die "Error, usage has changed.  Both strands are analyed by default.  Use -S for strand-specific (top-strand only) behavior.\n";
}

my $TOP_STRAND_ONLY = $opt_S || 0;
...
$cmd = "$UTIL_DIR/gene_list_to_gff.pl $acc_file $gff3_file.inx > best_candidates.gff3";
&process_cmd($cmd);

# exclude shadow orfs (smaller orfs in different reading frame that are eclipsed by longer orfs)
$cmd = "$UTIL_DIR/remove_eclipsed_ORFs.pl best_candidates.gff3 > best_candidates.eclipsed_orfs_removed.gff3";
&process_cmd($cmd);

foreach my $gff3_file ("best_candidates.gff3", "best_candidates.eclipsed_orfs_removed.gff3") {
