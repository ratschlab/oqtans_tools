...
for i in ${input1}.????; do
	...
	echo "python $basedir/palmapper_wrapper.py --input1=$i --input2=${input2}.$splitno --outsuffix=.${splitno} $* > $OQTANS_TMP_PATH/palmapper.$suffix.$splitno.log" | ${qsub} -cwd -N "plm$suffix$splitno" -pe "*" 1-4 -j y -o $OQTANS_TMP_PATH/palmapper.$suffix.$splitno.log
    fi
done
