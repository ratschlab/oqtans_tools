...
def bowtie2_paired(options):
	b2 = subprocess.Popen(['bowtie2', '-t', '-p', options.threads, '-X', options.insert, '-x', options.target, '-1', options.left,  '-2', options.right, '-U', options.unpaired, '-S', options.name], stdout=subprocess.PIPE)	
