...
if (SOLVER_MOSEK),
  ...
  if (sqrt(sum(alphas.*alphas)) <= length(f)*eps),
    warning('OPTIMIZER DELIVERED A NON-SOLUTION!');
    keyboard;
...
if (SOLVER_PRLOQO),
  ...
  %alphas2(inds) = alphas22;
  %warning on;
  %delta = sum((alphas2-alphas).^2);
  %fprintf('delta (alphas) %f and  #all/inds=%i/%i\n ',delta,size(H,1),length(inds));
  %if (delta>1e-2),
  %	fprintf('Difference too big (alphas): %f\n ',delta);
  %	keyboard
  %end
