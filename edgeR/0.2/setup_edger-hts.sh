...
echo ==========================================
echo  edgeR-hts setup script \(edgeR version $edgeR_VERSION\) 
echo ==========================================
echo
echo edgeR-hts base directory \(currently set to \"$edgeR_PATH\", suggest to set to \"`pwd`\", used if left empty\)
read edgeR_PATH
if [ "$edgeR_PATH" == "" ];
then
	edgeR_PATH=`pwd`
fi
echo '=>' Setting edgeR-hts base directory to \"$edgeR_PATH\"
echo
...
cp -p bin/edger_config.sh bin/edger_config.sh.bk
grep -v -e OCTAVE_BIN_PATH -e OCTAVE_MKOCT -e MATLAB_BIN_PATH -e MATLAB_MEX_PATH -e MATLAB_INCLUDE_DIR \
    -e edgeR_PATH -e edgeR_SRC_PATH -e edgeR_BIN_PATH \
    -e INTERPRETER -e SAMTOOLS_DIR -e PYTHON_PATH -e SCIPY_PATH -e R_PATH -e $edgeR_VERSION bin/edger_config.sh.bk  \
    > bin/edger_config.sh
echo
echo
echo generating config file

echo export edgeR_VERSION=$edgeR_VERSION >> bin/edger_config.sh
echo export edgeR_PATH=$edgeR_PATH >> bin/edger_config.sh
echo export edgeR_SRC_PATH=${edgeR_PATH}/src >> bin/edger_config.sh
echo export edgeR_BIN_PATH=${edgeR_PATH}/bin >> bin/edger_config.sh
echo export INTERPRETER=$INTERPRETER >> bin/edger_config.sh
echo export MATLAB_BIN_PATH=$MATLAB_BIN_PATH >> bin/edger_config.sh
echo export MATLAB_MEX_PATH=$MATLAB_MEX_PATH >> bin/edger_config.sh
echo export MATLAB_INCLUDE_DIR=$MATLAB_INCLUDE_DIR >> bin/edger_config.sh
echo export OCTAVE_BIN_PATH=$OCTAVE_BIN_PATH >> bin/edger_config.sh
echo export OCTAVE_MKOCT=$OCTAVE_MKOCT >> bin/edger_config.sh
echo export SAMTOOLS_DIR=$SAMTOOLS_DIR >> bin/edger_config.sh
echo export PYTHON_PATH=$PYTHON_PATH >> bin/edger_config.sh
echo export SCIPY_PATH=$SCIPY_PATH >> bin/edger_config.sh
echo export R_PATH=$R_PATH >> bin/edger_config.sh
