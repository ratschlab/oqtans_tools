function train_orgs(base_dir, in_dirs, out_dir, num_train, num_vald, params_c, ...
...
if (~PAR.mtl.mtl_enable),
  % save train/vald splitting for each repetition
  % if there is no mtl root available -> that means each mtl root must
  % supply a valid splitting
  splitting = cell(PAR.num_repetitions, 1);
end
...
for n = 1:PAR.num_repetitions,
  ...
  if (PAR.mtl.mtl_enable),
    [PAR, label, signal, state_label, exm_id_intervals] = ...
      load_multiple_organisms(PAR, base_dir, in_dirs, num_train, num_vald, splitting{n});
  else
    [PAR, label, signal, state_label, exm_id_intervals] = ...
      load_multiple_organisms(PAR, base_dir, in_dirs, num_train, num_vald);
    splitting{n} = PAR.organism;
  end
...
% if parallel training was enabled then collect the
% results here
if (PARALLEL),
  % eleminate trainset from PAR
  PAR.trainset = [];
...
save([out_dir '/final.mat'],'-v7.3','result','PAR','mtl_PAR','splitting','PARALLEL');
% eof