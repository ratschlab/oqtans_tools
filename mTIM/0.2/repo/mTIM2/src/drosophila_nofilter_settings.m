function CFG = drosophila_nofilter_settings(CFG)
...
%CFG.train_params = { ...
%  [0.1],   [0.5],   [0.05],  [1500], 'QP', [1]; ...  % max amount of train data available without val data
%};
...
% splice preds do not depend on filtered or unfiltered read alignments
...
read_map_file = [read_map_dir 'D_melanogaster_L3.6.bestpaired.bam'];
...
% competitor
CFG.cufflinks_pred_file = sprintf('%scufflinks/drosophila/D_melanogaster_L3.%s/sanitized_out/transcripts.gtf.mat', data_dir, '6.bestpaired');
