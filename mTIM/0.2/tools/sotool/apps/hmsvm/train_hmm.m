function progress = train_hmm(PAR, ...
  train_exm_ids, holdout_exm_ids, exm_id_intervals, signal, label, state_label, ...
  score_plifs, transition_scores, state_model, LABELS)
...
if ~isempty(holdout_exm_ids),
  for j=1:length(holdout_exm_ids),
    val_idx = find(exm_id_intervals(:,1)==train_exm_ids(j));
